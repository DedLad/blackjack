...
app.post('/api/start', async (req, res) => {
  ...
  try {
    ...
    if (user.money <= 0) {
      user.money = 700; // Reset money if the user is broke
    }

    user.wager = 0; // Reset wager on game start
    await user.save();
...
app.post('/api/stand', async (req, res) => {
  ...
  try {
    ...
    if (finalResult === 'win') {
      user.money += wager * 2; // Double the wager on win
    } else if (finalResult === 'lose') {
      user.money -= wager; // Subtract the wager on loss
    }

    // No money change for draw
    await user.save();
...
app.post('/api/setWager', async (req, res) => {
  ...
  try {
    ...
    res.json({ wager: user.wager, money: user.money }); // Return updated money and wager
  } catch (error) {
